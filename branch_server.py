...
class Branch(branch_pb2_grpc.BranchServicer):
    ...
    def handle_query_response(self):
        ...
        response_result = ResponseResult(status="success", balance=self.balance)
    ...
    def propagate_withdraw(self, amount_to_withdraw: int):
        propagate_withdraw_request = RequestElement(
            id=-1, interface="propagate_withdraw", money=amount_to_withdraw
        )
    ...
    def MsgDelivery(self, request, context):
        ...
        for request_element in request.request_elements:
            ...
            print(
                "Branch ID: {} handling Event ID:{}.Current balance:{}".format(
                    str(self.id), str(event_id), str(self.balance)
                ),
                "\n",
            )
...
def serve(branch_id: int, port: int, branch_ports: list):
    ...
    branch = Branch(id=branch_id, balance=0, branches=branch_ports)
    branch_pb2_grpc.add_BranchServicer_to_server(
        Branch(id=branch_id, balance=0, branches=branch_ports), server
    )
